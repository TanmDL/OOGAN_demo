...
class DisentangleGAN(nn.Module):
    def __init__(self, device, ngf, ndf, z_dim, c_dim, im_size, nc, g_type, d_type, prob_c, one_hot, lr, recon_weight, onehot_weight):
        ...
        self.device = device
        self.prob_c = prob_c
        self.one_hot = one_hot    ##### true for one hot-code vector
        self.g_type = g_type      ##### model type
        self.d_type = d_type

        self.recon_weight = recon_weight    #### both fixed to 1
        self.onehot_weight = onehot_weight  #### both fixed to 1
